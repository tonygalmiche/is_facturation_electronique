...
class AccountMove(models.Model):
    ...
    def generate_en16931_xml(self, flavor2level, pdf_invoice_bin=False):
        ...
        if force_saxon_validation:
            ...
            # generate_en16931_xml() above already ran this same schematron check
            # once per flavor, but silently on communication failures
            # (raise_if_http_error=False, hardcoded in the facturx lib and not
            # overridable from Odoo). We re-run it here with
            # raise_if_http_error=True to actually block the invoice when the
            # Saxon server is unreachable (a real validation failure, i.e. the
            # file genuinely fails the schematron, already raises on its own).
